...
- **2026-05-11** — **Catálogo enriquecido com nomes de jogadores + imagens reais das figurinhas**. Usuário forneceu 2 JSONs (Panini FIFA World Cup 2026 checklist completo + com URLs de imagens de `laststicker.com`). Preprocessado em `src/data/panini2026.json` (164 KB stripped: 20 intro + 960 base com `code/teamCode/number/name/team/type/imageUrl`). `worldCup2026.ts` agora gera o catálogo a partir do JSON. `Sticker` type estendido com `playerName`, `stickerType` (`player|logo|team_photo|intro`), `imageUrl`. **Modal do scanner** agora mostra a foto real da figurinha Panini + nome do jogador + país ("Hugo Ekitiké · France"), igual a referência. **Sticker card / Faltantes / Repetidas** mostram nome do jogador no lugar do país quando aplicável. `DATA_VERSION 6 → 7`. Bundle JS subiu de 666→839 KB (170 KB do catálogo). JSONs originais ignorados via `.gitignore`.
- **2026-05-11** — Scanner com **auto-scan + modal de preview dark**. ScannerPage.tsx ganhou `setInterval` rodando OCR a cada 1.1s no modo foto (sem precisar apertar Escanear); guard `scanningRef` evita re-entrada; `lastDetectedRef` cria cooldown de 4s pra mesma figurinha não pular modal repetido. Modal restilizado: card preview com bandeira grande + código mono + nome do país, input editável (`previewSticker` recalculado live conforme user corrige o código), botões Cancelar/Confirmar verde estilo referência. Removido botão "Escanear" do modo foto (auto-scan resolve). Novas classes `.scanner-modal/-close/-preview/-preview-card/-preview-card-code/-preview-code/-preview-name/-modal-input/-modal-actions/-modal-cancel/-modal-confirm`. **Limitação registrada**: face recognition por foto do jogador exige dataset de 782 fotos + modelo ML — não viável sem isso. Ofereci ao user pra adicionar quando tiver `{teamCode, number, name, photoUrl}` JSON.
